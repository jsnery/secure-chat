...
app.config['PROFILE_FOLDER'] = './app/data/upload/profiles'
extensoes_img = (
    '.png',
    '.jpg',
    '.jpeg',
...
def entrada():
    ...
    if secure_chat.get_profile_pic_url(usuario):
        profile_pic_url = f'/profiles/{session['id2']}.jpg'
    else:
        profile_pic_url = '/profiles/default.jpg'
    ...
    print(f"Usuario logado: {nickname}")
...
def friend():
    ...
    return render_template(
        ...
        friend_id=friend_id,
        get_profile_pic_url=get_friend_pic,
        bio=usuario.get_friend_bio(friend_nickname)
        )


@app.route('/entrada/edituser', methods=['GET', 'POST'])
def edit_username():
    usuario = set_user(
        session.get('password'),
        session.get('nickname'),
    ...
    if username is not None and not username.isspace() and len(username) <= 15 and ' ' not in username:
        usuario.user = username
        usuario.user_edit(usuario.nickname, username)
...
@app.route('/entrada/editbio', methods=['GET', 'POST'])
def edit_bio():
    usuario = set_user(
        session.get('password'),
        session.get('nickname'),
    ...
    if bio is not None and not bio.isspace() and len(bio) <= 50:
        usuario.set_bio(bio)
        return jsonify({'status': True})
...
def encrypt_server_name():
    ...
    try:
        ...
        if servidor_name is None:
            print("Nenhum nome de servidor fornecido")
            return jsonify(error="Nenhum nome de servidor fornecido"), 400

        print(f"Servidor: {servidor_name}")
        encryptedServer = encrypt(servidor_name, 40028922)
        print(f"Servidor encriptado: {encryptedServer}")
...
# Rota para a foto de perfil do usuário
@app.route('/profiles/<path:filename>')
def profile_pic(filename):
    return send_from_directory('data/upload/profiles', filename)
