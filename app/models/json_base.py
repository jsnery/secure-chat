...
class User:
    ...
    bio: str = 'Olá, bem bindo(a) ao Secure!'
    ...
    def get_friend_bio(self, friend_nickname: str):
        user = self.refuser.order_by_child(
    ...
    def user_edit(self, nickname: str, new_username: str):
        user = self.refuser.order_by_child(
            'nickname').equal_to(nickname).get()
        if user:
            for key in user.keys():
                self.refuser.child(key).update({
    ...
    def delete_db(self, server_name: str):
        ...
        file_path = f'app/data/chat/{self.encrypt(server_name.lower(), 40028922)}/{self.encrypt(server_name.lower(), 40028922)}.json'  # noqa
        ...
        backup_path = f'app/data/chat/{self.encrypt(server_name.lower(), 40028922)}/.backup/{self.encrypt(server_name.lower(), 40028922)}.json'  # noqa
    ...
    def verificar_se_existe_nova_mensagem(self, private_room: str):
        ...
        file_path = f'app/data/chat/{self.encrypt(private_room.lower(), 40028922)}/{self.encrypt(private_room.lower(), 40028922)}.json'  # noqa
                            ...
                            if os.path.exists(file_path):
                                with open(file_path, 'r') as f:
                                    try:
                                        ...
                                        print(f"Erro ao carregar arquivo {file_path}")
        return False
...
class SecureChat:
            ...
            with open(self.file_path, 'r') as f:
                if f.read().strip():
                    ...
                    try:
                        ...
                        print(f"Erro ao decodificar JSON do arquivo {self.file_path}")
    ...
    def delete_db(self, cod):
        if cod == 'delete':
            backup_file_path = f'app/data/chat/{self.encrypt(self.user.server_name.lower(), 40028922)}/.backup/{self.encrypt(self.user.server_name.lower(), 40028922)}.json'  # noqa
            # Faz backup das mensagens
